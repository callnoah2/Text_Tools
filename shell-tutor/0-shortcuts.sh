...
history_echo_cat_epilogue() {
	...
	the line, you will have noticed that didn't work.  The line editor
	relies $(bld entirely) on the keyboard to control the cursor.  This is due,
	in part, to the fact that mice weren't widespread when terminals were
	created.

	This remains true today because there are better ways to move the cursor
	than taking your hands away from the keyboard to point and click with
	another device.
...
ctrl_a_prologue() {
	if [[ $_OS = MacOSX ]]; then
		cat <<-:
		As big of an improvement that was, relying on the arrow keys to move
		to the beginning of the line still takes too long when your command
		is long.
		...
		** $(kbd M) represents the $(kbd Option) key
		...
		cat <<-:
		As big of an improvement that was, relying on the arrow keys to move
		to the beginning of the line still takes too long when the command
		is very long.
...
tab_prologue() {
	...
	$(kbd Tab); the shell will fill in $(kbd quentializing.txt).  
...
finish_hint() {
	if   (( _FAIL > 5 )) && [[ $1 == $WRONG_ARGS || $1 == $TOO_FEW_ARGS || $1 == $TOO_MANY_ARGS ]]; then
		cat <<-:
